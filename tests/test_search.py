...
from redis5.commands.json.path import Path
from redis5.commands.search import Search
from redis5.commands.search.field import (
    GeoField,
    NumericField,
    TagField,
    TextField,
...
from redis5.commands.search.indexDefinition import IndexDefinition, IndexType
...
@pytest.fixture
def client(request):
    r = _get_client(redis5.Redis, request, decode_responses=True)
    r.flushdb()
    return r
...
def test_aggregations_groupby(client):
    ...
    if is_resp2_connection(client):
        req = aggregations.AggregateRequest("redis5").group_by(
...
def test_dialect(client):
    ...
    with pytest.raises(redis5.ResponseError) as err:
...
@pytest.mark.redismod
def test_expire_while_search(client: redis5.Redis):
    client.ft().create_index((TextField("txt"),))
...
def test_withsuffixtrie(client: redis5.Redis):
    # create index
...
@pytest.mark.redismod
def test_query_timeout(r: redis5.Redis):
