...
class TestScripting:
    ...
    def test_eval_ro(self, r):
        r.set("a", "b")
        assert r.eval_ro("return redis5.call('GET', KEYS[1])", 1, "a") == b"b"
        with pytest.raises(redis5.ResponseError):
            r.eval_ro("return redis5.call('DEL', KEYS[1])", 1, "a")
    ...
    def test_eval_same_slot(self, r):
        """
        In a clustered redis5, the script keys must be in the same slot.

        This test isn't very interesting for standalone redis5, but it doesn't
        hurt anything.
        """
        r.set("A{foo}", 2)
        r.set("B{foo}", 4)
        ...
        script = """
        local value = redis5.call('GET', KEYS[1])
        local value2 = redis5.call('GET', KEYS[2])
        return value * value2
        """
    ...
    def test_evalsha_ro(self, r):
        r.set("a", "b")
        get_sha = r.script_load("return redis5.call('GET', KEYS[1])")
        del_sha = r.script_load("return redis5.call('DEL', KEYS[1])")
    ...
    def test_script_object(self, r):
        ...
        # Test that the precalculated sha matches the one from redis5
    ...
    def test_script_object_in_pipeline(self, r):
        ...
        # purge the script from redis5's cache and re-run the pipeline
