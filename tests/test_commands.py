...
class TestRedisCommands:
    ...
    def test_acl_log(self, r, request):
        ...
        user_client = _get_client(
            redis5.Redis, request, flushdb=False, username=username
        )
    ...
    def test_client_list_client_id(self, r, request):
        ...
        _get_client(redis5.Redis, request, flushdb=False)
        _get_client(redis5.Redis, request, flushdb=False)
        _get_client(redis5.Redis, request, flushdb=False)
    ...
    def test_client_setinfo(self, r: redis5.Redis):
        r.ping()
        info = r.client_info()
        ...
        r2 = redis5.Redis(lib_name="test2", lib_version="1234")
        info = r2.client_info()
        assert info["lib-name"] == "test2"
        assert info["lib-ver"] == "1234"
        r3 = redis5.Redis(lib_name=None, lib_version=None)
        info = r3.client_info()
    ...
    def test_client_kill(self, r, r2):
        r.client_setname("redis5-py-c1")
        r2.client_setname("redis5-py-c2")
        clients = [
            client
    ...
    def test_client_kill_filter_by_laddr(self, r, r2):
        ...
        client_2_addr = clients_by_name["redis5-py-c2"].get("laddr")
    ...
    def test_client_kill_filter_by_user(self, r, request):
        ...
        _get_client(redis5.Redis, request, flushdb=False, username=killuser)
    ...
    @skip_if_server_version_lt("7.0.0")
    def test_config_get_multi_params(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("7.0.0")
    @skip_if_redis_enterprise()
    def test_config_set_multi_params(self, r: redis5.Redis):
    ...
    def test_never_decode_option(self, r: redis5.Redis):
        opts = {NEVER_DECODE: []}
        r.delete("a")
        assert r.execute_command("EXISTS", "a", **opts) == 0

    def test_empty_response_option(self, r: redis5.Redis):
        opts = {EMPTY_RESPONSE: []}
        r.delete("a")
        assert r.execute_command("EXISTS", "a", **opts) == 0
    ...
    def test_copy_to_another_database(self, request):
        r0 = _get_client(redis5.Redis, request, db=0)
        r1 = _get_client(redis5.Redis, request, db=1)
        r0.set("a", "foo")
    ...
    def test_stralgo_lcs(self, r):
        ...
        if skip_if_redis_enterprise().args[0] is True:
            with pytest.raises(redis5.exceptions.ResponseError):
    ...
    def generate_lib_code(self, lib_name):
        return f"""#!js api_version=1.0 name={lib_name}\n redis5.registerFunction('foo', ()=>{{return 'bar'}})"""  # noqa
    ...
    def test_tfunction_list(self, r):
        ...
        with pytest.raises(redis5.exceptions.DataError):
    ...
    def test_zmpop(self, r):
        ...
        with pytest.raises(redis5.DataError):
    ...
    @skip_if_server_version_lt("7.2.0")
    def test_zrank_withscore(self, r: redis5.Redis):
        r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
        assert r.zrank("a", "a1") == 0
    ...
    @skip_if_server_version_lt("7.0.0")
    def test_xadd_explicit_ms(self, r: redis5.Redis):
        stream = "stream"
    ...
    @skip_if_server_version_lt("7.0.0")
    def test_xgroup_create_entriesread(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("6.0.0")
    def test_bitfield_ro(self, r: redis5.Redis):
        bf = r.bitfield("a")
    ...
    @skip_if_server_version_lt("7.0.0")
    def test_latency_histogram_not_implemented(self, r: redis5.Redis):
        with pytest.raises(NotImplementedError):
            r.latency_histogram()

    def test_latency_graph_not_implemented(self, r: redis5.Redis):
        with pytest.raises(NotImplementedError):
            r.latency_graph()

    def test_latency_doctor_not_implemented(self, r: redis5.Redis):
        with pytest.raises(NotImplementedError):
            r.latency_doctor()

    def test_latency_history(self, r: redis5.Redis):
        assert r.latency_history("command") == []

    def test_latency_latest(self, r: redis5.Redis):
        assert r.latency_latest() == []

    def test_latency_reset(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("7.0.0")
    @skip_if_redis_enterprise()
    def test_command_list(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("7.0.0")
    @skip_if_redis_enterprise()
    def test_command_getkeysandflags(self, r: redis5.Redis):
        assert_resp_response(
            r,
    ...
    def test_module(self, r):
        with pytest.raises(redis5.exceptions.ModuleError) as excinfo:
    ...
    @skip_if_server_version_lt("7.0.0")
    @skip_if_redis_enterprise()
    def test_module_loadex(self, r: redis5.Redis):
        with pytest.raises(redis5.exceptions.ModuleError) as excinfo:
    ...
    def test_shutdown(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("7.0.0")
    def test_shutdown_with_params(self, r: redis5.Redis):
        r.execute_command = mock.MagicMock()
    ...
    def test_sync(self, r):
        r.flushdb()
        time.sleep(1)
        r2 = redis5.Redis(port=6380, decode_responses=False)
        res = r2.sync()
    ...
    @pytest.mark.onlynoncluster
    def test_interrupted_command(self, r: redis5.Redis):
        """
