...
from redis5.asyncio.connection import async_timeout
    ...
    async def handle(self, reader, writer):
        ...
        try:
            pipe1 = asyncio.create_task(
                self.pipe(reader, redis_writer, "to redis5:", self.send_event)
            )
            pipe2 = asyncio.create_task(self.pipe(redis_reader, writer, "from redis5:"))
    ...
    async with contextlib.AsyncExitStack() as stack:
        ...
        with contextlib.closing(
            RedisCluster.from_url(
                f"redis5://127.0.0.1:{remap_base}", address_remap=remap
            )
        ) as r:
            await r.initialize()
            await r.set("foo", "foo")
