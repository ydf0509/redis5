...
import redis5
from redis5 import exceptions
from redis5._parsers.helpers import (
    _RedisCallbacks,
    _RedisCallbacksRESP2,
    _RedisCallbacksRESP3,
    parse_info,
)
from redis5.client import EMPTY_RESPONSE, NEVER_DECODE
...
async def r_teardown(r: redis5.Redis):
    """
...
@pytest_asyncio.fixture()
async def slowlog(r: redis5.Redis):
...
async def redis_server_time(client: redis5.Redis):
    seconds, milliseconds = await client.time()
    timestamp = float(f"{seconds}.{milliseconds}")
    return datetime.datetime.fromtimestamp(timestamp)


async def get_stream_message(client: redis5.Redis, stream: str, message_id: str):
...
class TestResponseCallbacks:
    ...
    async def test_response_callbacks(self, r: redis5.Redis):
    ...
    async def test_case_insensitive_command_names(self, r: redis5.Redis):
        assert r.response_callbacks["ping"] == r.response_callbacks["PING"]


class TestRedisCommands:
    async def test_command_on_invalid_key_type(self, r: redis5.Redis):
        await r.lpush("a", "1")
        with pytest.raises(redis5.ResponseError):
            await r.get("a")

    # SERVER INFORMATION
    @skip_if_server_version_lt(REDIS_6_VERSION)
    async def test_acl_cat_no_category(self, r: redis5.Redis):
        categories = await r.acl_cat()
        assert isinstance(categories, list)
        assert "read" in categories or b"read" in categories

    @skip_if_server_version_lt(REDIS_6_VERSION)
    async def test_acl_cat_with_category(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt(REDIS_6_VERSION)
    async def test_acl_genpass(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt(REDIS_6_VERSION)
    async def test_acl_users(self, r: redis5.Redis):
        users = await r.acl_users()
        assert isinstance(users, list)
        assert len(users) > 0

    @skip_if_server_version_lt(REDIS_6_VERSION)
    async def test_acl_whoami(self, r: redis5.Redis):
        username = await r.acl_whoami()
        assert isinstance(username, (str, bytes))

    @pytest.mark.onlynoncluster
    async def test_client_list(self, r: redis5.Redis):
        clients = await r.client_list()
        assert isinstance(clients[0], dict)
        assert "addr" in clients[0]

    @skip_if_server_version_lt("5.0.0")
    async def test_client_list_type(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("5.0.0")
    @pytest.mark.onlynoncluster
    async def test_client_id(self, r: redis5.Redis):
        assert await r.client_id() > 0

    @skip_if_server_version_lt("5.0.0")
    @pytest.mark.onlynoncluster
    async def test_client_unblock(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_client_getname(self, r: redis5.Redis):
        assert await r.client_getname() is None

    @skip_if_server_version_lt("2.6.9")
    @pytest.mark.onlynoncluster
    async def test_client_setname(self, r: redis5.Redis):
    ...
    async def test_client_setinfo(self, r: redis5.Redis):
        await r.ping()
        info = await r.client_info()
        assert info["lib-name"] == "redis5-py"
        assert info["lib-ver"] == redis5.__version__
        ...
        r2 = redis5.asyncio.Redis(lib_name="test2", lib_version="1234")
        info = await r2.client_info()
        assert info["lib-name"] == "test2"
        assert info["lib-ver"] == "1234"
        r3 = redis5.asyncio.Redis(lib_name=None, lib_version=None)
    ...
    @skip_if_server_version_lt("2.6.9")
    @pytest.mark.onlynoncluster
    async def test_client_kill(self, r: redis5.Redis, r2):
        await r.client_setname("redis5-py-c1")
        await r2.client_setname("redis5-py-c2")
        clients = [
            client
            for client in await r.client_list()
            if client.get("name") in ["redis5-py-c1", "redis5-py-c2"]
        ]
        assert len(clients) == 2

        clients_by_name = {client.get("name"): client for client in clients}

        client_addr = clients_by_name["redis5-py-c2"].get("addr")
        ...
        assert clients[0].get("name") == "redis5-py-c1"

    @skip_if_server_version_lt("2.8.12")
    async def test_client_kill_filter_invalid_params(self, r: redis5.Redis):
        # empty
        with pytest.raises(exceptions.DataError):
    ...
    @skip_if_server_version_lt("2.8.12")
    @pytest.mark.onlynoncluster
    async def test_client_kill_filter_by_id(self, r: redis5.Redis, r2):
        await r.client_setname("redis5-py-c1")
        await r2.client_setname("redis5-py-c2")
        clients = [
            client
        ...
        client_2_id = clients_by_name["redis5-py-c2"].get("id")
    ...
    @skip_if_server_version_lt("2.8.12")
    @pytest.mark.onlynoncluster
    async def test_client_kill_filter_by_addr(self, r: redis5.Redis, r2):
        await r.client_setname("redis5-py-c1")
        await r2.client_setname("redis5-py-c2")
        clients = [
            client
        ...
        client_2_addr = clients_by_name["redis5-py-c2"].get("addr")
    ...
    @skip_if_server_version_lt("2.6.9")
    async def test_client_list_after_client_setname(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_client_pause(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("7.2.0")
    @pytest.mark.onlynoncluster
    async def test_client_no_touch(self, r: redis5.Redis):
    ...
    async def test_config_get(self, r: redis5.Redis):
        data = await r.config_get()
        assert "maxmemory" in data
        assert data["maxmemory"].isdigit()

    @pytest.mark.onlynoncluster
    async def test_config_resetstat(self, r: redis5.Redis):
        await r.ping()
    ...
    async def test_config_set(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_dbsize(self, r: redis5.Redis):
        await r.set("a", "foo")
        await r.set("b", "bar")
        assert await r.dbsize() == 2

    @pytest.mark.onlynoncluster
    async def test_echo(self, r: redis5.Redis):
        assert await r.echo("foo bar") == b"foo bar"

    @pytest.mark.onlynoncluster
    async def test_info(self, r: redis5.Redis):
        await r.set("a", "foo")
        await r.set("b", "bar")
        info = await r.info()
    ...
    @pytest.mark.onlynoncluster
    async def test_lastsave(self, r: redis5.Redis):
        assert isinstance(await r.lastsave(), datetime.datetime)

    async def test_object(self, r: redis5.Redis):
        await r.set("a", "foo")
    ...
    async def test_ping(self, r: redis5.Redis):
        assert await r.ping()

    @pytest.mark.onlynoncluster
    async def test_slowlog_get(self, r: redis5.Redis, slowlog):
    ...
    @pytest.mark.onlynoncluster
    async def test_slowlog_get_limit(self, r: redis5.Redis, slowlog):
        assert await r.slowlog_reset()
        await r.get("foo")
    ...
    @pytest.mark.onlynoncluster
    async def test_slowlog_length(self, r: redis5.Redis, slowlog):
        await r.get("foo")
        assert isinstance(await r.slowlog_len(), int)

    @skip_if_server_version_lt("2.6.0")
    async def test_time(self, r: redis5.Redis):
        t = await r.time()
        assert len(t) == 2
        assert isinstance(t[0], int)
        assert isinstance(t[1], int)

    async def test_never_decode_option(self, r: redis5.Redis):
        opts = {NEVER_DECODE: []}
        await r.delete("a")
        assert await r.execute_command("EXISTS", "a", **opts) == 0

    async def test_empty_response_option(self, r: redis5.Redis):
        opts = {EMPTY_RESPONSE: []}
        await r.delete("a")
        assert await r.execute_command("EXISTS", "a", **opts) == 0

    # BASIC KEY COMMANDS
    async def test_append(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_bitcount(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    @pytest.mark.onlynoncluster
    async def test_bitop_not_empty_string(self, r: redis5.Redis):
        await r.set("a", "")
    ...
    @skip_if_server_version_lt("2.6.0")
    @pytest.mark.onlynoncluster
    async def test_bitop_not(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    @pytest.mark.onlynoncluster
    async def test_bitop_not_in_place(self, r: redis5.Redis):
        test_str = b"\xAA\x00\xFF\x55"
        correct = ~0xAA00FF55 & 0xFFFFFFFF
        await r.set("a", test_str)
    ...
    @skip_if_server_version_lt("2.6.0")
    @pytest.mark.onlynoncluster
    async def test_bitop_single_string(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    @pytest.mark.onlynoncluster
    async def test_bitop_string_operands(self, r: redis5.Redis):
    ...
    async def test_bitpos(self, r: redis5.Redis):
        key = "key:bitpos"
    ...
    @skip_if_server_version_lt("2.8.7")
    async def test_bitpos_wrong_arguments(self, r: redis5.Redis):
    ...
    async def test_decr(self, r: redis5.Redis):
    ...
    async def test_decrby(self, r: redis5.Redis):
        assert await r.decrby("a", amount=2) == -2
        assert await r.decrby("a", amount=3) == -5
        assert await r.get("a") == b"-5"

    async def test_delete(self, r: redis5.Redis):
        assert await r.delete("a") == 0
        await r.set("a", "foo")
        assert await r.delete("a") == 1

    async def test_delete_with_multiple_keys(self, r: redis5.Redis):
        await r.set("a", "foo")
        await r.set("b", "bar")
    ...
    async def test_delitem(self, r: redis5.Redis):
        await r.set("a", "foo")
        await r.delete("a")
        assert await r.get("a") is None

    @skip_if_server_version_lt("4.0.0")
    async def test_unlink(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("4.0.0")
    async def test_unlink_with_multiple_keys(self, r: redis5.Redis):
        await r.set("a", "foo")
        await r.set("b", "bar")
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_dump_and_restore(self, r: redis5.Redis):
        await r.set("a", "foo")
    ...
    async def test_dump_and_restore_and_replace(self, r: redis5.Redis):
        await r.set("a", "bar")
        dumped = await r.dump("a")
        with pytest.raises(redis5.ResponseError):
            await r.restore("a", 0, dumped)
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_dump_and_restore_absttl(self, r: redis5.Redis):
        await r.set("a", "foo")
        dumped = await r.dump("a")
        await r.delete("a")
        ttl = int(
    ...
    async def test_exists(self, r: redis5.Redis):
    ...
    async def test_exists_contains(self, r: redis5.Redis):
        assert not await r.exists("a")
        await r.set("a", "foo")
        assert await r.exists("a")

    async def test_expire(self, r: redis5.Redis):
    ...
    async def test_expireat_datetime(self, r: redis5.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        await r.set("a", "foo")
        assert await r.expireat("a", expire_at)
        assert 0 < await r.ttl("a") <= 61

    async def test_expireat_no_key(self, r: redis5.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        assert not await r.expireat("a", expire_at)

    async def test_expireat_unixtime(self, r: redis5.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        await r.set("a", "foo")
    ...
    async def test_get_and_set(self, r: redis5.Redis):
    ...
    async def test_get_set_bit(self, r: redis5.Redis):
        # no value
    ...
    async def test_incrby(self, r: redis5.Redis):
        assert await r.incrby("a") == 1
        assert await r.incrby("a", 4) == 5
        assert await r.get("a") == b"5"

    @skip_if_server_version_lt("2.6.0")
    async def test_incrbyfloat(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_keys(self, r: redis5.Redis):
        assert await r.keys() == []
    ...
    @pytest.mark.onlynoncluster
    async def test_mget(self, r: redis5.Redis):
        assert await r.mget([]) == []
    ...
    @pytest.mark.onlynoncluster
    async def test_mset(self, r: redis5.Redis):
        d = {"a": b"1", "b": b"2", "c": b"3"}
    ...
    @pytest.mark.onlynoncluster
    async def test_msetnx(self, r: redis5.Redis):
        d = {"a": b"1", "b": b"2", "c": b"3"}
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_pexpire(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_pexpireat_datetime(self, r: redis5.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        await r.set("a", "foo")
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_pexpireat_no_key(self, r: redis5.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        assert not await r.pexpireat("a", expire_at)

    @skip_if_server_version_lt("2.6.0")
    async def test_pexpireat_unixtime(self, r: redis5.Redis):
        expire_at = await redis_server_time(r) + datetime.timedelta(minutes=1)
        await r.set("a", "foo")
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_psetex(self, r: redis5.Redis):
        assert await r.psetex("a", 1000, "value")
        assert await r.get("a") == b"value"
        assert 0 < await r.pttl("a") <= 1000

    @skip_if_server_version_lt("2.6.0")
    async def test_psetex_timedelta(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_pttl(self, r: redis5.Redis):
    ...
    async def test_pttl_no_key(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_randomkey(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_rename(self, r: redis5.Redis):
        await r.set("a", "1")
    ...
    @pytest.mark.onlynoncluster
    async def test_renamenx(self, r: redis5.Redis):
        await r.set("a", "1")
        await r.set("b", "2")
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_set_nx(self, r: redis5.Redis):
        assert await r.set("a", "1", nx=True)
        assert not await r.set("a", "2", nx=True)
        assert await r.get("a") == b"1"

    @skip_if_server_version_lt("2.6.0")
    async def test_set_xx(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_set_px(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_set_px_timedelta(self, r: redis5.Redis):
        expire_at = datetime.timedelta(milliseconds=1000)
    ...
    @skip_if_server_version_lt("2.6.0")
    async def test_set_ex(self, r: redis5.Redis):
        assert await r.set("a", "1", ex=10)
        assert 0 < await r.ttl("a") <= 10

    @skip_if_server_version_lt("2.6.0")
    async def test_set_ex_timedelta(self, r: redis5.Redis):
        expire_at = datetime.timedelta(seconds=60)
        assert await r.set("a", "1", ex=expire_at)
        assert 0 < await r.ttl("a") <= 60

    @skip_if_server_version_lt("2.6.0")
    async def test_set_multipleoptions(self, r: redis5.Redis):
        await r.set("a", "val")
        assert await r.set("a", "1", xx=True, px=10000)
        assert 0 < await r.ttl("a") <= 10

    @skip_if_server_version_lt(REDIS_6_VERSION)
    async def test_set_keepttl(self, r: redis5.Redis):
        await r.set("a", "val")
        assert await r.set("a", "1", xx=True, px=10000)
        assert 0 < await r.ttl("a") <= 10
    ...
    async def test_setex(self, r: redis5.Redis):
        assert await r.setex("a", 60, "1")
        assert await r.get("a") == b"1"
        assert 0 < await r.ttl("a") <= 60

    async def test_setnx(self, r: redis5.Redis):
        assert await r.setnx("a", "1")
        assert await r.get("a") == b"1"
        assert not await r.setnx("a", "2")
        assert await r.get("a") == b"1"

    async def test_setrange(self, r: redis5.Redis):
    ...
    async def test_strlen(self, r: redis5.Redis):
        await r.set("a", "foo")
        assert await r.strlen("a") == 3

    async def test_substr(self, r: redis5.Redis):
    ...
    async def test_ttl(self, r: redis5.Redis):
        await r.set("a", "1")
        assert await r.expire("a", 10)
        assert 0 < await r.ttl("a") <= 10
        assert await r.persist("a")
    ...
    # LIST COMMANDS
    @pytest.mark.onlynoncluster
    async def test_blpop(self, r: redis5.Redis):
        await r.rpush("a", "1", "2")
        await r.rpush("b", "3", "4")
        assert_resp_response(
    ...
    @pytest.mark.onlynoncluster
    async def test_brpop(self, r: redis5.Redis):
        await r.rpush("a", "1", "2")
        await r.rpush("b", "3", "4")
        assert_resp_response(
    ...
    @pytest.mark.onlynoncluster
    async def test_brpoplpush(self, r: redis5.Redis):
        await r.rpush("a", "1", "2")
        await r.rpush("b", "3", "4")
    ...
    @pytest.mark.onlynoncluster
    async def test_brpoplpush_empty_string(self, r: redis5.Redis):
        await r.rpush("a", "")
        assert await r.brpoplpush("a", "b") == b""

    async def test_lindex(self, r: redis5.Redis):
        await r.rpush("a", "1", "2", "3")
        assert await r.lindex("a", "0") == b"1"
        assert await r.lindex("a", "1") == b"2"
        assert await r.lindex("a", "2") == b"3"

    async def test_linsert(self, r: redis5.Redis):
        await r.rpush("a", "1", "2", "3")
    ...
    async def test_llen(self, r: redis5.Redis):
        await r.rpush("a", "1", "2", "3")
        assert await r.llen("a") == 3

    async def test_lpop(self, r: redis5.Redis):
        await r.rpush("a", "1", "2", "3")
    ...
    async def test_lpush(self, r: redis5.Redis):
    ...
    async def test_lpushx(self, r: redis5.Redis):
    ...
    async def test_lrange(self, r: redis5.Redis):
    ...
    async def test_lrem(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_rpoplpush(self, r: redis5.Redis):
    ...
    async def test_rpush(self, r: redis5.Redis):
    ...
    async def test_lpos(self, r: redis5.Redis):
    ...
    async def test_rpushx(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.8.0")
    @pytest.mark.onlynoncluster
    async def test_scan(self, r: redis5.Redis):
        await r.set("a", 1)
        await r.set("b", 2)
        await r.set("c", 3)
    ...
    @skip_if_server_version_lt(REDIS_6_VERSION)
    @pytest.mark.onlynoncluster
    async def test_scan_type(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.8.0")
    @pytest.mark.onlynoncluster
    async def test_scan_iter(self, r: redis5.Redis):
        await r.set("a", 1)
        await r.set("b", 2)
        await r.set("c", 3)
    ...
    @skip_if_server_version_lt("2.8.0")
    async def test_sscan(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.8.0")
    async def test_sscan_iter(self, r: redis5.Redis):
        await r.sadd("a", 1, 2, 3)
    ...
    @skip_if_server_version_lt("2.8.0")
    async def test_hscan(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.8.0")
    async def test_hscan_iter(self, r: redis5.Redis):
        await r.hset("a", mapping={"a": 1, "b": 2, "c": 3})
    ...
    @skip_if_server_version_lt("2.8.0")
    async def test_zscan(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("2.8.0")
    async def test_zscan_iter(self, r: redis5.Redis):
        await r.zadd("a", {"a": 1, "b": 2, "c": 3})
    ...
    # SET COMMANDS
    async def test_sadd(self, r: redis5.Redis):
        members = {b"1", b"2", b"3"}
        await r.sadd("a", *members)
        assert await r.smembers("a") == members

    async def test_scard(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
        assert await r.scard("a") == 3

    @pytest.mark.onlynoncluster
    async def test_sdiff(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    @pytest.mark.onlynoncluster
    async def test_sdiffstore(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    @pytest.mark.onlynoncluster
    async def test_sinter(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    @pytest.mark.onlynoncluster
    async def test_sinterstore(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    async def test_sismember(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
    ...
    async def test_smembers(self, r: redis5.Redis):
        await r.sadd("a", "1", "2", "3")
        assert await r.smembers("a") == {b"1", b"2", b"3"}

    @pytest.mark.onlynoncluster
    async def test_smove(self, r: redis5.Redis):
    ...
    async def test_spop(self, r: redis5.Redis):
        s = [b"1", b"2", b"3"]
        await r.sadd("a", *s)
    ...
    async def test_spop_multi_value(self, r: redis5.Redis):
        s = [b"1", b"2", b"3"]
        await r.sadd("a", *s)
    ...
    async def test_srandmember(self, r: redis5.Redis):
        s = [b"1", b"2", b"3"]
        await r.sadd("a", *s)
        assert await r.srandmember("a") in s

    @skip_if_server_version_lt("2.6.0")
    async def test_srandmember_multi_value(self, r: redis5.Redis):
        s = [b"1", b"2", b"3"]
        await r.sadd("a", *s)
    ...
    async def test_srem(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_sunion(self, r: redis5.Redis):
        await r.sadd("a", "1", "2")
        await r.sadd("b", "2", "3")
        assert await r.sunion("a", "b") == {b"1", b"2", b"3"}

    @pytest.mark.onlynoncluster
    async def test_sunionstore(self, r: redis5.Redis):
        await r.sadd("a", "1", "2")
        await r.sadd("b", "2", "3")
    ...
    # SORTED SET COMMANDS
    async def test_zadd(self, r: redis5.Redis):
    ...
    async def test_zadd_nx(self, r: redis5.Redis):
    ...
    async def test_zadd_incr(self, r: redis5.Redis):
        assert await r.zadd("a", {"a1": 1}) == 1
        assert await r.zadd("a", {"a1": 4.5}, incr=True) == 5.5

    async def test_zadd_incr_with_xx(self, r: redis5.Redis):
    ...
    async def test_zcard(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
        assert await r.zcard("a") == 3

    async def test_zcount(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    async def test_zincrby(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @pytest.mark.onlynoncluster
    async def test_zinterstore_max(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 1, "a3": 1})
        await r.zadd("b", {"a1": 2, "a2": 2, "a3": 2})
        await r.zadd("c", {"a1": 6, "a3": 5, "a4": 4})
    ...
    @pytest.mark.onlynoncluster
    async def test_zinterstore_min(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @pytest.mark.onlynoncluster
    async def test_zinterstore_with_weight(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 1, "a3": 1})
        await r.zadd("b", {"a1": 2, "a2": 2, "a3": 2})
        await r.zadd("c", {"a1": 6, "a3": 5, "a4": 4})
    ...
    async def test_zpopmax(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @skip_if_server_version_lt("4.9.0")
    async def test_zpopmin(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @skip_if_server_version_lt("4.9.0")
    @pytest.mark.onlynoncluster
    async def test_bzpopmax(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("4.9.0")
    @pytest.mark.onlynoncluster
    async def test_bzpopmin(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2})
        await r.zadd("b", {"b1": 10, "b2": 20})
        assert_resp_response(
            r,
    ...
    async def test_zrange(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @skip_if_server_version_lt("2.8.9")
    async def test_zrangebylex(self, r: redis5.Redis):
        await r.zadd("a", {"a": 0, "b": 0, "c": 0, "d": 0, "e": 0, "f": 0, "g": 0})
    ...
    async def test_zrevrangebylex(self, r: redis5.Redis):
        await r.zadd("a", {"a": 0, "b": 0, "c": 0, "d": 0, "e": 0, "f": 0, "g": 0})
    ...
    async def test_zrangebyscore(self, r: redis5.Redis):
    ...
    async def test_zrank(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    @skip_if_server_version_lt("7.2.0")
    async def test_zrank_withscore(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
        assert await r.zrank("a", "a1") == 0
    ...
    async def test_zrem(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    async def test_zrem_multiple_keys(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
        assert await r.zrem("a", "a1", "a2") == 2
        assert await r.zrange("a", 0, 5) == [b"a3"]

    @skip_if_server_version_lt("2.8.9")
    async def test_zremrangebylex(self, r: redis5.Redis):
        await r.zadd("a", {"a": 0, "b": 0, "c": 0, "d": 0, "e": 0, "f": 0, "g": 0})
    ...
    async def test_zremrangebyrank(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
        assert await r.zremrangebyrank("a", 1, 3) == 3
        assert await r.zrange("a", 0, 5) == [b"a1", b"a5"]

    async def test_zremrangebyscore(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    async def test_zrevrange(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    async def test_zrevrangebyscore(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    async def test_zrevrank(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
    ...
    @skip_if_server_version_lt("7.2.0")
    async def test_zrevrank_withscore(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3, "a4": 4, "a5": 5})
        assert await r.zrevrank("a", "a1") == 4
        assert await r.zrevrank("a", "a2") == 3
        assert await r.zrevrank("a", "a6") is None
    ...
    async def test_zscore(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @pytest.mark.onlynoncluster
    async def test_zunionstore_sum(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 1, "a3": 1})
        await r.zadd("b", {"a1": 2, "a2": 2, "a3": 2})
        await r.zadd("c", {"a1": 6, "a3": 5, "a4": 4})
    ...
    @pytest.mark.onlynoncluster
    async def test_zunionstore_max(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 1, "a3": 1})
        await r.zadd("b", {"a1": 2, "a2": 2, "a3": 2})
        await r.zadd("c", {"a1": 6, "a3": 5, "a4": 4})
    ...
    @pytest.mark.onlynoncluster
    async def test_zunionstore_min(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 2, "a3": 3})
    ...
    @pytest.mark.onlynoncluster
    async def test_zunionstore_with_weight(self, r: redis5.Redis):
        await r.zadd("a", {"a1": 1, "a2": 1, "a3": 1})
        await r.zadd("b", {"a1": 2, "a2": 2, "a3": 2})
        await r.zadd("c", {"a1": 6, "a3": 5, "a4": 4})
    ...
    # HYPERLOGLOG TESTS
    @skip_if_server_version_lt("2.8.9")
    async def test_pfadd(self, r: redis5.Redis):
        members = {b"1", b"2", b"3"}
    ...
    @skip_if_server_version_lt("2.8.9")
    @pytest.mark.onlynoncluster
    async def test_pfcount(self, r: redis5.Redis):
        members = {b"1", b"2", b"3"}
    ...
    @skip_if_server_version_lt("2.8.9")
    @pytest.mark.onlynoncluster
    async def test_pfmerge(self, r: redis5.Redis):
        mema = {b"1", b"2", b"3"}
        memb = {b"2", b"3", b"4"}
        memc = {b"5", b"6", b"7"}
    ...
    async def test_hset_with_multi_key_values(self, r: redis5.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
        assert await r.hget("a", "1") == b"1"
        assert await r.hget("a", "2") == b"2"
        assert await r.hget("a", "3") == b"3"
    ...
    async def test_hset_without_data(self, r: redis5.Redis):
        with pytest.raises(exceptions.DataError):
            await r.hset("x")

    async def test_hdel(self, r: redis5.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
    ...
    async def test_hlen(self, r: redis5.Redis):
        await r.hset("a", mapping={"1": 1, "2": 2, "3": 3})
        assert await r.hlen("a") == 3

    async def test_hmget(self, r: redis5.Redis):
        assert await r.hset("a", mapping={"a": 1, "b": 2, "c": 3})
        assert await r.hmget("a", "a", "b", "c") == [b"1", b"2", b"3"]

    async def test_hmset(self, r: redis5.Redis):
        warning_message = (
    ...
    async def test_hsetnx(self, r: redis5.Redis):
    ...
    async def test_hvals(self, r: redis5.Redis):
        h = {b"a1": b"1", b"a2": b"2", b"a3": b"3"}
        await r.hset("a", mapping=h)
    ...
    @pytest.mark.onlynoncluster
    async def test_sort_get(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_sort_get_multi(self, r: redis5.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
        await r.rpush("a", "2", "3", "1")
    ...
    @pytest.mark.onlynoncluster
    async def test_sort_get_groups_two(self, r: redis5.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
        await r.rpush("a", "2", "3", "1")
    ...
    @pytest.mark.onlynoncluster
    async def test_sort_groups_string_get(self, r: redis5.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
        await r.rpush("a", "2", "3", "1")
    ...
    @pytest.mark.onlynoncluster
    async def test_sort_groups_just_one_get(self, r: redis5.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
        await r.rpush("a", "2", "3", "1")
    ...
    async def test_sort_groups_no_get(self, r: redis5.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
        await r.rpush("a", "2", "3", "1")
    ...
    @pytest.mark.onlynoncluster
    async def test_sort_groups_three_gets(self, r: redis5.Redis):
        await r.set("user:1", "u1")
        await r.set("user:2", "u2")
        await r.set("user:3", "u3")
    ...
    async def test_sort_issue_924(self, r: redis5.Redis):
    ...
    @pytest.mark.onlynoncluster
    async def test_readwrite(self, r: redis5.Redis):
        assert await r.readwrite()

    @skip_if_server_version_lt("3.0.0")
    @pytest.mark.onlynoncluster
    async def test_readonly_invalid_cluster_state(self, r: redis5.Redis):
        with pytest.raises(exceptions.RedisError):
            await r.readonly()
    ...
    # GEO COMMANDS
    @skip_if_server_version_lt("3.2.0")
    async def test_geoadd(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_geoadd_invalid_params(self, r: redis5.Redis):
        with pytest.raises(exceptions.RedisError):
            await r.geoadd("barcelona", (1, 2))

    @skip_if_server_version_lt("3.2.0")
    async def test_geodist(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_geodist_units(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_geodist_missing_one_member(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1")
        await r.geoadd("barcelona", values)
        assert await r.geodist("barcelona", "place1", "missing_member", "km") is None

    @skip_if_server_version_lt("3.2.0")
    async def test_geodist_invalid_units(self, r: redis5.Redis):
        with pytest.raises(exceptions.RedisError):
            assert await r.geodist("x", "y", "z", "inches")

    @skip_if_server_version_lt("3.2.0")
    async def test_geohash(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_geopos(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
        ...
        await r.geoadd("barcelona", values)
        # redis5 uses 52 bits precision, hereby small errors may be introduced.
        assert_resp_response(
            r,
    ...
    @skip_if_server_version_lt("4.0.0")
    async def test_geopos_no_value(self, r: redis5.Redis):
        assert await r.geopos("barcelona", "place1", "place2") == [None, None]

    @skip_if_server_version_lt("3.2.0")
    @skip_if_server_version_gte("4.0.0")
    async def test_old_geopos_no_value(self, r: redis5.Redis):
        assert await r.geopos("barcelona", "place1", "place2") == []

    @skip_if_server_version_lt("3.2.0")
    async def test_georadius(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            b"\x80place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_georadius_no_values(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_georadius_units(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_georadius_with(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_georadius_count(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    async def test_georadius_sort(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    @pytest.mark.onlynoncluster
    async def test_georadius_store(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_if_server_version_lt("3.2.0")
    @pytest.mark.onlynoncluster
    async def test_georadius_store_dist(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            "place2",
    ...
    @skip_unless_arch_bits(64)
    @skip_if_server_version_lt("3.2.0")
    async def test_georadiusmember(self, r: redis5.Redis):
        values = (2.1909389952632, 41.433791470673, "place1") + (
            2.1873744593677,
            41.406342043777,
            b"\x80place2",
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xack(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer = "consumer"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xadd(self, r: redis5.Redis):
        stream = "stream"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xclaim(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer1 = "consumer1"
        consumer2 = "consumer2"
    ...
    @skip_if_server_version_lt("7.0.0")
    async def test_xclaim_trimmed(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xdel(self, r: redis5.Redis):
        stream = "stream"
    ...
    @skip_if_server_version_lt("7.0.0")
    async def test_xgroup_create(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("7.0.0")
    async def test_xgroup_create_mkstream(self, r: redis5.Redis):
        # tests xgroup_create and xinfo_groups
        stream = "stream"
        group = "group"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xgroup_delconsumer(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer = "consumer"
        await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xgroup_destroy(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("7.0.0")
    async def test_xgroup_setid(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        message_id = await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("7.2.0")
    async def test_xinfo_consumers(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer1 = "consumer1"
        consumer2 = "consumer2"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xinfo_stream(self, r: redis5.Redis):
        stream = "stream"
        m1 = await r.xadd(stream, {"foo": "bar"})
        m2 = await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xlen(self, r: redis5.Redis):
        stream = "stream"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xpending(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer1 = "consumer1"
        consumer2 = "consumer2"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xpending_range(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer1 = "consumer1"
        consumer2 = "consumer2"
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xrange(self, r: redis5.Redis):
        stream = "stream"
        m1 = await r.xadd(stream, {"foo": "bar"})
        m2 = await r.xadd(stream, {"foo": "bar"})
        m3 = await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xread(self, r: redis5.Redis):
        stream = "stream"
        m1 = await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xreadgroup(self, r: redis5.Redis):
        stream = "stream"
        group = "group"
        consumer = "consumer"
        m1 = await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xrevrange(self, r: redis5.Redis):
        stream = "stream"
        m1 = await r.xadd(stream, {"foo": "bar"})
        m2 = await r.xadd(stream, {"foo": "bar"})
        m3 = await r.xadd(stream, {"foo": "bar"})
    ...
    @skip_if_server_version_lt("5.0.0")
    async def test_xtrim(self, r: redis5.Redis):
        stream = "stream"
    ...
    @pytest.mark.onlynoncluster
    async def test_bitfield_operations(self, r: redis5.Redis):
    ...
    async def test_bitfield_ro(self, r: redis5.Redis):
        bf = r.bitfield("a")
    ...
    @skip_if_server_version_lt("4.0.0")
    async def test_memory_stats(self, r: redis5.Redis):
    ...
    @skip_if_server_version_lt("4.0.0")
    async def test_memory_usage(self, r: redis5.Redis):
        await r.set("foo", "bar")
        assert isinstance(await r.memory_usage("foo"), int)

    @skip_if_server_version_lt("4.0.0")
    @pytest.mark.onlynoncluster
    async def test_module_list(self, r: redis5.Redis):
        assert isinstance(await r.module_list(), list)
        for x in await r.module_list():
            assert isinstance(x, dict)

    @pytest.mark.onlynoncluster
    async def test_interrupted_command(self, r: redis5.Redis):
        """
...
class TestBinarySave:
    async def test_binary_get_set(self, r: redis5.Redis):
    ...
    async def test_binary_lists(self, r: redis5.Redis):
        mapping = {
    ...
    async def test_22_info(self, r: redis5.Redis):
        """
    ...
    async def test_large_responses(self, r: redis5.Redis):
    ...
    async def test_floating_point_encoding(self, r: redis5.Redis):
        """
