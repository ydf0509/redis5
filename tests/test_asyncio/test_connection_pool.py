...
from redis5.asyncio.connection import Connection, to_bool
...
class TestConnectionPoolURLParsing:
    def test_hostname(self):
        pool = redis.ConnectionPool.from_url("redis5://my.host")
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {"host": "my.host"}

    def test_quoted_hostname(self):
        pool = redis.ConnectionPool.from_url("redis5://my %2F host %2B%3D+")
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {"host": "my / host +=+"}

    def test_port(self):
        pool = redis.ConnectionPool.from_url("redis5://localhost:6380")
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {"host": "localhost", "port": 6380}

    @skip_if_server_version_lt("6.0.0")
    def test_username(self):
        pool = redis.ConnectionPool.from_url("redis5://myuser:@localhost")
        assert pool.connection_class == redis.Connection
    ...
    def test_password(self):
        pool = redis.ConnectionPool.from_url("redis5://:mypassword@localhost")
        assert pool.connection_class == redis.Connection
    ...
    def test_username_and_password(self):
        pool = redis.ConnectionPool.from_url("redis5://myuser:mypass@localhost")
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "username": "myuser",
    ...
    def test_db_as_argument(self):
        pool = redis.ConnectionPool.from_url("redis5://localhost", db=1)
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {"host": "localhost", "db": 1}

    def test_db_in_path(self):
        pool = redis.ConnectionPool.from_url("redis5://localhost/2", db=1)
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {"host": "localhost", "db": 2}

    def test_db_in_querystring(self):
        pool = redis.ConnectionPool.from_url("redis5://localhost/2?db=3", db=1)
        assert pool.connection_class == redis.Connection
    ...
    def test_client_name_in_querystring(self):
        pool = redis.ConnectionPool.from_url("redis5://location?client_name=test-client")
    ...
    def test_extra_querystring_options(self):
        pool = redis.ConnectionPool.from_url("redis5://localhost?a=1&b=2")
        assert pool.connection_class == redis.Connection
        assert pool.connection_kwargs == {"host": "localhost", "a": "1", "b": "2"}

    def test_calling_from_subclass_returns_correct_instance(self):
        pool = redis.BlockingConnectionPool.from_url("redis5://localhost")
        assert isinstance(pool, redis.BlockingConnectionPool)

    def test_client_creates_connection_pool(self):
        r = redis.Redis.from_url("redis5://myhost")
...
class TestConnection:
    ...
    def test_connect_from_url_tcp(self):
        connection = redis.Redis.from_url("redis5://localhost")
