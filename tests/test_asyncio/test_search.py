...
import pytest
import redis5.asyncio as redis
import redis5.commands.search
import redis5.commands.search.aggregation as aggregations
import redis5.commands.search.reducers as reducers
from redis5.commands.search import AsyncSearch
from redis5.commands.search.field import GeoField, NumericField, TagField, TextField
from redis5.commands.search.indexDefinition import IndexDefinition
from redis5.commands.search.query import GeoFilter, NumericFilter, Query
from redis5.commands.search.result import Result
from redis5.commands.search.suggestion import Suggestion
...
def getClient(decoded_r: redis5.Redis):
    """
...
async def test_client(decoded_r: redis5.Redis):
    num_docs = 500
...
async def test_scores(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_stopwords(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_filters(decoded_r: redis5.Redis):
    await (
...
@pytest.mark.redismod
async def test_sort_by(decoded_r: redis5.Redis):
    await (
        decoded_r.ft().create_index(
...
async def test_drop_index(decoded_r: redis5.Redis):
    """
...
@pytest.mark.redismod
async def test_example(decoded_r: redis5.Redis):
    ...
    await decoded_r.hset(
        ...
        mapping={
            "title": "RediSearch",
            "body": "Redisearch impements a search engine on top of redis5",
        },
    )
...
@pytest.mark.redismod
async def test_auto_complete(decoded_r: redis5.Redis):
    n = 0
...
@pytest.mark.redismod
async def test_no_index(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_explain(decoded_r: redis5.Redis):
    await (
...
@pytest.mark.redismod
async def test_explaincli(decoded_r: redis5.Redis):
    with pytest.raises(NotImplementedError):
        await decoded_r.ft().explain_cli("foo")


@pytest.mark.redismod
async def test_summarize(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
@skip_ifmodversion_lt("2.0.0", "search")
async def test_alias(decoded_r: redis5.Redis):
...
async def test_alias_basic(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_tags(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_textfield_sortable_nostem(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_alter_schema_add(decoded_r: redis5.Redis):
    # Creating the index definition and schema
...
@pytest.mark.redismod
async def test_spell_check(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_dict_operations(decoded_r: redis5.Redis):
    await decoded_r.ft().create_index((TextField("f1"), TextField("f2")))
    # Add three items
...
@pytest.mark.redismod
async def test_phonetic_matcher(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
@pytest.mark.onlynoncluster
async def test_scorer(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_get(decoded_r: redis5.Redis):
    await decoded_r.ft().create_index((TextField("f1"), TextField("f2")))
...
async def test_config(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
@pytest.mark.onlynoncluster
async def test_aggregations_groupby(decoded_r: redis5.Redis):
    # Creating the index definition and schema
    await decoded_r.ft().create_index(
        (
    ...
    for dialect in [1, 2]:
        if is_resp2_connection(decoded_r):
            req = (
                aggregations.AggregateRequest("redis5")
            ...
            res = (await decoded_r.ft().aggregate(req)).rows[0]
            assert res == ["parent", "redis5", "first", "RediSearch"]
            ...
            assert res["extra_attributes"]["parent"] == "redis5"
            ...
            res = (await decoded_r.ft().aggregate(req))["results"][0]
            assert res["extra_attributes"] == {"parent": "redis5", "first": "RediSearch"}
...
@pytest.mark.redismod
async def test_aggregations_sort_by_and_limit(decoded_r: redis5.Redis):
...
async def test_withsuffixtrie(decoded_r: redis5.Redis):
    # create index
...
@pytest.mark.redismod
@skip_if_redis_enterprise()
async def test_search_commands_in_pipeline(decoded_r: redis5.Redis):
...
@pytest.mark.redismod
async def test_query_timeout(decoded_r: redis5.Redis):
