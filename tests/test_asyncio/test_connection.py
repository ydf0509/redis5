...
import pytest
import redis5
from redis5._parsers import (
    _AsyncHiredisParser,
    _AsyncRESP2Parser,
    _AsyncRESP3Parser,
    _AsyncRESPBase,
)
from redis5.asyncio import Redis
from redis5.asyncio.connection import Connection, UnixDomainSocketConnection, parse_url
from redis5.asyncio.retry import Retry
from redis5.backoff import NoBackoff
from redis5.exceptions import ConnectionError, InvalidResponse, TimeoutError
...
async def test_loading_external_modules(r):
    ...
    # and call it
    from redis5.commands import RedisModuleCommands
...
@pytest.mark.onlynoncluster
async def test_connection_parse_response_resume(r: redis5.Redis):
    """
...
def test_create_single_connection_client_from_url():
    client = Redis.from_url("redis5://localhost:6379/0?", single_connection_client=True)
...
async def test_pool_auto_close(request, from_url):
    ...
    url: str = request.config.getoption("--redis5-url")
