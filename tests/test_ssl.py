...
import pytest
import redis5
from redis5.exceptions import ConnectionError, RedisError
...
class TestSSL:
    ...
    This relies on the --redis5-ssl-url purely for rebuilding the client
    ...
    def test_ssl_with_invalid_cert(self, request):
        ...
        sslclient = redis5.from_url(ssl_url)
    ...
    def test_ssl_connection(self, request):
        ...
        r = redis5.Redis(host=p[0], port=p[1], ssl=True, ssl_cert_reqs="none")
        assert r.ping()

    def test_ssl_connection_without_ssl(self, request):
        ssl_url = request.config.option.redis_ssl_url
        p = urlparse(ssl_url)[1].split(":")
        r = redis5.Redis(host=p[0], port=p[1], ssl=False)
    ...
    def test_valid_ocsp_cert_http(self):
        from redis5.ocsp import OCSPVerifier
