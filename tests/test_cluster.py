...
from redis5.connection import BlockingConnectionPool, Connection, ConnectionPool
from redis5.crc import key_slot
...
class TestRedisClusterObj:
    ...
    def test_from_url(self, r):
        redis_url = f"redis5://{default_host}:{default_port}/0"
        ...
        with patch.object(Redis, "parse_response") as parse_response:
            ...
            for node_name, conn in node_conn_origin.items():
                if node_name == node.name:
                    # The old redis5 connection of the timed out node should have been
                    # deleted and replaced
                    assert conn != r.get_redis_connection(node)
                else:
                    # other nodes' redis5 connection should have been reused during the
                    # topology refresh
...
class TestClusterRedisCommands:
    ...
    def test_client_kill(self, r, r2):
        node = r.get_primaries()[0]
        r.client_setname("redis5-py-c1", target_nodes="all")
        r2.client_setname("redis5-py-c2", target_nodes="all")
        clients = [
            client
...
class TestClusterPubSubObject:
    ...
    def test_get_redis_connection(self, r):
        """
        Test that get_redis_connection() returns the redis5 connection of the
        set pubsub node
        """
        node = r.get_default_node()
        p = r.pubsub(node=node)
