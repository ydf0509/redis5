...
import pytest
import redis5
from redis5.exceptions import ConnectionError
from redis5.utils import HIREDIS_AVAILABLE
...
class TestPubSubAutoDecoding:
    ...
    @pytest.fixture()
    def r(self, request):
        return _get_client(redis5.Redis, request=request, decode_responses=True)
...
class TestPubSubRedisDown:
    def test_channel_subscribe(self, r):
        r = redis5.Redis(host="localhost", port=6390)
        p = r.pubsub()
...
class TestPubSubDeadlock:
    ...
    def test_pubsub_deadlock(self, master_host):
        pool = redis5.ConnectionPool(host=master_host[0], port=master_host[1])
        r = redis5.Redis(connection_pool=pool)
...
class TestPubSubAutoReconnect:
    ...
    def test_reconnect_socket_error(self, r: redis5.Redis, method):
        """
    ...
    def test_reconnect_disconnect(self, r: redis5.Redis, method):
        """
...
class TestBaseException:
    def test_base_exception(self, r: redis5.Redis):
        """
        ...
        assert is_connected()
        with patch("redis5._parsers._RESP2Parser.read_response") as mock1, patch(
            "redis5._parsers._HiredisParser.read_response"
        ) as mock2, patch("redis5._parsers._RESP3Parser.read_response") as mock3:
