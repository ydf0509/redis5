...
import pytest
import redis5.commands.bf
from redis5.exceptions import ModuleError, RedisError
...
def client(decoded_r):
    assert isinstance(decoded_r.bf(), redis5.commands.bf.BFBloom)
    assert isinstance(decoded_r.cf(), redis5.commands.bf.CFBloom)
    assert isinstance(decoded_r.cms(), redis5.commands.bf.CMSBloom)
    assert isinstance(decoded_r.tdigest(), redis5.commands.bf.TDigestBloom)
    assert isinstance(decoded_r.topk(), redis5.commands.bf.TOPKBloom)
