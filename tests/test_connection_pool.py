...
import pytest
import redis5
from redis5.connection import to_bool
from redis5.utils import SSL_AVAILABLE
...
class TestConnectionPool:
    def get_pool(
        ...
        connection_class=redis5.Connection,
    ):
        connection_kwargs = connection_kwargs or {}
        pool = redis5.ConnectionPool(
    ...
    def test_max_connections(self, master_host):
        ...
        pool.get_connection("_")
        with pytest.raises(redis5.ConnectionError):
            pool.get_connection("_")
    ...
    def test_repr_contains_db_info_tcp(self):
        ...
        pool = self.get_pool(
            connection_kwargs=connection_kwargs, connection_class=redis5.Connection
        )
        expected = (
    ...
    def test_repr_contains_db_info_unix(self):
        ...
        pool = self.get_pool(
            ...
            connection_class=redis5.UnixDomainSocketConnection,
        )
        expected = (
...
class TestBlockingConnectionPool:
    def get_pool(self, connection_kwargs=None, max_connections=10, timeout=20):
        connection_kwargs = connection_kwargs or {}
        pool = redis5.BlockingConnectionPool(
...
class TestConnectionPoolURLParsing:
    def test_hostname(self):
        pool = redis5.ConnectionPool.from_url("redis5://my.host")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "my.host"}

    def test_quoted_hostname(self):
        pool = redis5.ConnectionPool.from_url("redis5://my %2F host %2B%3D+")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "my / host +=+"}

    def test_port(self):
        pool = redis5.ConnectionPool.from_url("redis5://localhost:6380")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "port": 6380}

    @skip_if_server_version_lt("6.0.0")
    def test_username(self):
        pool = redis5.ConnectionPool.from_url("redis5://myuser:@localhost")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "username": "myuser"}

    @skip_if_server_version_lt("6.0.0")
    def test_quoted_username(self):
        pool = redis5.ConnectionPool.from_url(
            "redis5://%2Fmyuser%2F%2B name%3D%24+:@localhost"
        )
        assert pool.connection_class == redis5.Connection
    ...
    def test_password(self):
        pool = redis5.ConnectionPool.from_url("redis5://:mypassword@localhost")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "password": "mypassword"}

    def test_quoted_password(self):
        pool = redis5.ConnectionPool.from_url(
            "redis5://:%2Fmypass%2F%2B word%3D%24+@localhost"
        )
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
    ...
    def test_username_and_password(self):
        pool = redis5.ConnectionPool.from_url("redis5://myuser:mypass@localhost")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {
            "host": "localhost",
            "username": "myuser",
    ...
    def test_db_as_argument(self):
        pool = redis5.ConnectionPool.from_url("redis5://localhost", db=1)
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "db": 1}

    def test_db_in_path(self):
        pool = redis5.ConnectionPool.from_url("redis5://localhost/2", db=1)
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "db": 2}

    def test_db_in_querystring(self):
        pool = redis5.ConnectionPool.from_url("redis5://localhost/2?db=3", db=1)
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "db": 3}

    def test_extra_typed_querystring_options(self):
        pool = redis5.ConnectionPool.from_url(
            "redis5://localhost/2?socket_timeout=20&socket_connect_timeout=10"
    ...
    def test_client_name_in_querystring(self):
        pool = redis5.ConnectionPool.from_url("redis5://location?client_name=test-client")
        assert pool.connection_kwargs["client_name"] == "test-client"

    def test_invalid_extra_typed_querystring_options(self):
        with pytest.raises(ValueError):
            redis5.ConnectionPool.from_url(
                "redis5://localhost/2?socket_timeout=_&socket_connect_timeout=abc"
            )

    def test_extra_querystring_options(self):
        pool = redis5.ConnectionPool.from_url("redis5://localhost?a=1&b=2")
        assert pool.connection_class == redis5.Connection
        assert pool.connection_kwargs == {"host": "localhost", "a": "1", "b": "2"}

    def test_calling_from_subclass_returns_correct_instance(self):
        pool = redis5.BlockingConnectionPool.from_url("redis5://localhost")
        assert isinstance(pool, redis5.BlockingConnectionPool)

    def test_client_creates_connection_pool(self):
        r = redis5.Redis.from_url("redis5://myhost")
        assert r.connection_pool.connection_class == redis5.Connection
        assert r.connection_pool.connection_kwargs == {"host": "myhost"}

    def test_invalid_scheme_raises_error(self):
        with pytest.raises(ValueError) as cm:
            redis5.ConnectionPool.from_url("localhost")
        assert str(cm.value) == (
    ...
    def test_invalid_scheme_raises_error_when_double_slash_missing(self):
        with pytest.raises(ValueError) as cm:
            redis5.ConnectionPool.from_url("redis5:foo.bar.com:12345")
        assert str(cm.value) == (
            "Redis URL must specify one of the following schemes "
            "(redis5://, rediss://, unix://)"
        )
...
class TestConnectionPoolUnixSocketURLParsing:
    def test_defaults(self):
        pool = redis5.ConnectionPool.from_url("unix:///socket")
        assert pool.connection_class == redis5.UnixDomainSocketConnection
        assert pool.connection_kwargs == {"path": "/socket"}

    @skip_if_server_version_lt("6.0.0")
    def test_username(self):
        pool = redis5.ConnectionPool.from_url("unix://myuser:@/socket")
        assert pool.connection_class == redis5.UnixDomainSocketConnection
    ...
    def test_password(self):
        pool = redis5.ConnectionPool.from_url("unix://:mypassword@/socket")
        assert pool.connection_class == redis5.UnixDomainSocketConnection
    ...
    def test_db_as_argument(self):
        pool = redis5.ConnectionPool.from_url("unix:///socket", db=1)
        assert pool.connection_class == redis5.UnixDomainSocketConnection
        assert pool.connection_kwargs == {"path": "/socket", "db": 1}

    def test_db_in_querystring(self):
        pool = redis5.ConnectionPool.from_url("unix:///socket?db=2", db=1)
        assert pool.connection_class == redis5.UnixDomainSocketConnection
    ...
    def test_extra_querystring_options(self):
        pool = redis5.ConnectionPool.from_url("unix:///socket?a=1&b=2")
        assert pool.connection_class == redis5.UnixDomainSocketConnection
        assert pool.connection_kwargs == {"path": "/socket", "a": "1", "b": "2"}

    def test_connection_class_override(self):
        class MyConnection(redis5.UnixDomainSocketConnection):
            pass
...
class TestSSLConnectionURLParsing:
    def test_host(self):
        pool = redis5.ConnectionPool.from_url("rediss://my.host")
        assert pool.connection_class == redis5.SSLConnection
        assert pool.connection_kwargs == {"host": "my.host"}

    def test_connection_class_override(self):
        class MyConnection(redis5.SSLConnection):
            pass
    ...
    def test_cert_reqs_options(self):
        ...
        class DummyConnectionPool(redis5.ConnectionPool):
...
class TestConnection:
    def test_on_connect_error(self):
        ...
        # 9999 databases ;)
        bad_connection = redis5.Redis(db=9999)
        # an error should be raised on connect
        with pytest.raises(redis5.RedisError):
            bad_connection.info()
    ...
    def test_busy_loading_disconnects_socket(self, r):
        ...
        """
        with pytest.raises(redis5.BusyLoadingError):
    ...
    def test_read_only_error(self, r):
        ...
        with pytest.raises(redis5.ReadOnlyError):
    ...
    def test_oom_error(self, r):
        ...
        with pytest.raises(redis5.OutOfMemoryError):
    ...
    def test_connect_from_url_tcp(self):
        connection = redis5.Redis.from_url("redis5://localhost")
    ...
    def test_connect_from_url_unix(self):
        connection = redis5.Redis.from_url("unix:///path/to/socket")
        pool = connection.connection_pool
    ...
    def test_connect_no_auth_configured(self, r):
        ...
        """
        # Redis < 6
        with pytest.raises(redis5.AuthenticationError):
            r.execute_command(
...
class TestMultiConnectionClient:
    @pytest.fixture()
    def r(self, request):
        return _get_client(redis5.Redis, request, single_connection_client=False)
...
class TestHealthCheck:
    ...
    @pytest.fixture()
    def r(self, request):
        return _get_client(redis5.Redis, request, health_check_interval=self.interval)
