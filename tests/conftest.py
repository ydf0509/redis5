...
from redis5.backoff import NoBackoff
from redis5.connection import parse_url
from redis5.exceptions import RedisClusterException
from redis5.retry import Retry

REDIS_INFO = {}
default_redis_url = "redis5://localhost:6379/0"
default_protocol = "2"
default_redismod_url = "redis5://localhost:6379"
...
def _get_info(redis_url):
    client = redis5.Redis.from_url(redis_url)
    info = client.info()
    try:
        client.execute_command("DPING")
        info["enterprise"] = True
    except redis5.ResponseError:
...
def pytest_sessionstart(session):
    ...
    redis_url = session.config.getoption("--redis5-url")
    try:
    ...
    except (KeyError, redis5.exceptions.ConnectionError):
        pass

    if cluster_enabled:
        cluster_nodes = session.config.getoption("--redis5-cluster-nodes")
...
def wait_for_cluster_creation(redis_url, cluster_nodes, timeout=60):
    ...
    considered ready.
    :param redis_url: the cluster's url, e.g. redis5://localhost:16379/0
    ...
    while now < end_time:
        try:
            client = redis5.RedisCluster.from_url(redis_url)
...
def skip_ifmodversion_lt(min_version: str, module_name: str):
    ...
    if modules == []:
        return pytest.mark.skipif(True, reason="No redis5 modules found")
    ...
    raise AttributeError(f"No redis5 module named {module_name}")
...
def skip_ifnot_redis_enterprise() -> _TestDecorator:
    ...
    return pytest.mark.skipif(check, reason="Not running in redis5 enterprise")
...
def _get_client(
    ...
    Uses the "--redis5-url" command line argument for connection info. Unlike
    ConnectionPool.from_url, keyword arguments to this function override
    values specified in the URL.
    """
    if from_url is None:
        redis_url = request.config.getoption("--redis5-url")
    else:
        redis_url = from_url
    ...
    if not cluster_mode:
        ...
        pool = redis5.ConnectionPool(**url_options)
        client = cls(connection_pool=pool)
    else:
        client = redis5.RedisCluster.from_url(redis_url, **kwargs)
...
@pytest.fixture()
def r(request):
    with _get_client(redis5.Redis, request) as client:
        yield client


@pytest.fixture()
def decoded_r(request):
    with _get_client(redis5.Redis, request, decode_responses=True) as client:
        yield client


@pytest.fixture()
def r_timeout(request):
    with _get_client(redis5.Redis, request, socket_timeout=1) as client:
        yield client
...
def sslclient(request):
    with _get_client(redis5.Redis, request, ssl=True) as client:
        yield client
...
def mock_cluster_resp_ok(request, **kwargs):
    r = _get_client(redis5.Redis, request, **kwargs)
...
def master_host(request):
    url = request.config.getoption("--redis5-url")
    parts = urlparse(url)
...
def is_resp2_connection(r):
    if isinstance(r, redis5.Redis) or isinstance(r, redis5.asyncio.Redis):
        protocol = r.connection_pool.connection_kwargs.get("protocol")
    elif isinstance(r, redis5.cluster.AbstractRedisCluster):
